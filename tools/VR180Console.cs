...
public class ConsoleForm : Form
{
    ...
    NumericUpDown syncOffsetNud;
    CheckBox stabilizeBox;
    ...
    public ConsoleForm(string[] args)
    {
        ...
        // Sync offset (K): the capture->ingest delay (OBS+NVENC+RTMP) that the
        // stabilization feature subtracts from the pose lookup. Shares /settings
        // with the headset page's own numeric field - calibrate from here without
        // putting the headset on, watching for drift while the player turns at a
        // constant rate (see docs/STABILIZATION.md).
        syncOffsetNud = new NumericUpDown { Width = 70, Minimum = 0, Maximum = 500, Increment = 1, Value = 50 };
        syncOffsetNud.ValueChanged += (s, e) => PushSyncOffset();

        // Mirrors the headset page's own checkbox through the same /settings bridge -
        // lets you toggle stabilization (and see it take effect on next Enter VR)
        // without touching the headset.
        stabilizeBox = new CheckBox {
            Text = "Stabilize to player's head", ForeColor = Ink, AutoSize = true, BackColor = Bg,
            Margin = new Padding(4, 6, 0, 0),
        };
        stabilizeBox.CheckedChanged += (s, e) => PushStabilize();
        ...
        cfgFlow.Controls.AddRange(new Control[] {
            ...
            MakeField("Sync offset (ms)", MakeRow(syncOffsetNud, stabilizeBox)),
        });
    ...
    void StartPipeline(string[] args)
    {
        ...
        rsLoadTimer.Tick += (s, e) => { rsLoadTimer.Stop(); LoadRenderScale(); LoadSyncOffset(); LoadStabilize(); };
        rsLoadTimer.Start();
    ...
    void PushSyncOffset()
    {
        string json = "{\"syncOffsetMs\":" + syncOffsetNud.Value.ToString("0", System.Globalization.CultureInfo.InvariantCulture) + "}";
        System.Threading.Tasks.Task.Run(() => HttpPostJson("http://127.0.0.1:9080/settings", json, 1500));
    }
    ...
    void LoadSyncOffset()
    {
        System.Threading.Tasks.Task.Run(() => {
            string body = HttpGet("http://127.0.0.1:9080/settings", 1500);
            if (body == null) return;
            var m = Regex.Match(body, "\"syncOffsetMs\"\\s*:\\s*([0-9.]+)");
            if (!m.Success) return;
            decimal v;
            if (decimal.TryParse(m.Groups[1].Value, System.Globalization.NumberStyles.Any,
                    System.Globalization.CultureInfo.InvariantCulture, out v)) {
                v = Math.Min(500m, Math.Max(0m, v));
                if (syncOffsetNud.InvokeRequired) syncOffsetNud.BeginInvoke((Action)(() => syncOffsetNud.Value = v));
                else syncOffsetNud.Value = v;
            }
        });
    }
    ...
    void PushStabilize()
    {
        string json = "{\"stabilize\":" + (stabilizeBox.Checked ? "true" : "false") + "}";
        System.Threading.Tasks.Task.Run(() => HttpPostJson("http://127.0.0.1:9080/settings", json, 1500));
    }
    ...
    void LoadStabilize()
    {
        System.Threading.Tasks.Task.Run(() => {
            string body = HttpGet("http://127.0.0.1:9080/settings", 1500);
            if (body == null) return;
            var m = Regex.Match(body, "\"stabilize\"\\s*:\\s*(true|false)");
            if (!m.Success) return;
            bool v = m.Groups[1].Value == "true";
            if (stabilizeBox.InvokeRequired) stabilizeBox.BeginInvoke((Action)(() => stabilizeBox.Checked = v));
            else stabilizeBox.Checked = v;
        });
    }
